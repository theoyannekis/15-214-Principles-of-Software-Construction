 ...
 * Generic, Sudoku X, Hypersudoku, and Asterisk Sudoku
 */
public abstract class SudokuSolutionVerifier {
     ...
     * types being "Generic", "Hyper", "X", and "Asterisk"
    ...
    public static void main(String[] args) throws FileNotFoundException, IllegalArgumentException {
        ...
        if (args.length < NUMARGS) {
            throw new IllegalArgumentException("Must have 3 command line arguments, File1, File2, " +
                    "puzzleType. PuzzleType may be 'Generic', 'Hyper', 'X', or 'Asterisk'");
        }
        String puzzleType = args[2];
        switch (puzzleType) {
            case "Asterisk": {
                AsteriskSolutionVerifier verifier = new AsteriskSolutionVerifier(puzzle, propSol);
                System.out.println(verifier.isValidSudokuSolution());
                break;
            }
            case "X": {
            ...
            default: {
                throw new IllegalArgumentException("PuzzleType must be 'Generic', 'Hyper', 'X', or 'Asterisk'");
            }
        }
     ...
     */
    SudokuSolutionVerifier(SudokuPuzzleGrid puz, SudokuPuzzleGrid pSol) {
        puzzle = puz;
        propSol = pSol;
    ...
    /**
     * Returns the validity of the proposed solution as a solution for that specific puzzle type. For example,
     * if it is an asterisk puzzle then this function returns true if the board follows the Asterisk rules
     * @return boolean representing the validity of the board as an answer to that specific puzzle type
     */
    abstract boolean isSpecificPuzzleType();
     ...
     */
    boolean isValidSudokuSolution() {
        for (int i = 0; i < BOARDSIZE; i++) {
            if (!isRowValid(i) || !isColValid(i) || !isBoxValid(i)) {
                return false;
            }
        }
        return isMatch() & isSpecificPuzzleType();
    }
